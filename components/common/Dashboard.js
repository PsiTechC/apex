...
export default function Dashboard({ ownerEmails = [], organizationType }) {
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        <KpiCard title="Total Controls" value={total} />
        <KpiCard title="Compliance Score" value={`${complianceScore}%`} />
        <KpiCard title="Pending Approval" value={counts["pending-approval"] || 0} />
        <KpiCard title="Pending" value={counts.pending || 0} />
        <KpiCard
          title="RE Type"
          value={
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card title="Owners">
          <OwnersList emails={ownerEmails} pendingByEmail={pendingByEmail} />
        </Card>
      </div>
    </div>
  );
...
function OwnersList({ emails = [], pendingByEmail = {} }) {
  const [active, setActive] = useState(null);
  const hoverInTid = useRef(null);
  const hoverOutTid = useRef(null);

  const items = [...new Set((emails || []).filter(Boolean).map((e) => e.trim()))];

  if (!items.length) {
    return <p className="text-sm text-gray-500">No owners provided.</p>;
  }
  ...
  const handleEnter = (email) => {
    ...
    hoverInTid.current = setTimeout(() => setActive(email), 80); // hover-in delay
  };
